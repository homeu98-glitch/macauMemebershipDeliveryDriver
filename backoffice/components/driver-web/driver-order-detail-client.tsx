        ...
        <section className="android-card detail-main-card stack gap-3 no-overflow-card full-width-card">
          <div className="driver-inline-between align-start gap-3 orders-header-row">
            ...
            <div className="stack gap-2 align-end">
              ...
              {order.chat?.enabled && order.chat.messagesUrl ? <DriverChatIconButton className="driver-chat-icon-under-price" hasUnread={chatUnread.hasUnread(order.id)} onClick={() => { void chatUnread.refresh(true); setChatOpen(true); chatUnread.markRead(order.id, order.chat); }} /> : null}
